...
class _LockManager(object):
    ...
    def __init__(self, celery_self, timeout, include_args, args, kwargs):
        ...
        self.args = args
        self.kwargs = kwargs
        self.log = getLogger('{0}:{1}'.format(
            self.__class__.__name__, self.task_identifier))
    ...
    def task_identifier(self):
        ...
        if self.include_args:
            merged_args = str(
                self.args) + str([(k, self.kwargs[k]) for k in sorted(self.kwargs)])
            task_id += '.args.{0}'.format(hashlib.md5(
                merged_args.encode('utf-8')).hexdigest())
        return task_id
...
class _LockManagerRedis(_LockManager):
    ...
    def __init__(self, celery_self, timeout, include_args, args, kwargs):
        super(_LockManagerRedis, self).__init__(
            celery_self, timeout, include_args, args, kwargs)
        self.lock = None

    def __enter__(self):
        redis_key = self.CELERY_LOCK.format(task_id=self.task_identifier)
        self.lock = self.celery_self.backend.client.lock(
        ...
        if not self.lock.acquire(blocking=False):
            ...
            raise OtherInstanceError(
                'Failed to acquire lock, {0} already running.'.format(self.task_identifier))
        else:
...
class _LockManagerDB(_LockManager):
    ...
    def __init__(self, celery_self, timeout, include_args, args, kwargs):
        super(_LockManagerDB, self).__init__(
            celery_self, timeout, include_args, args, kwargs)
        self.save_group = getattr(self.celery_self.backend, '_save_group')
        self.restore_group = getattr(
            self.celery_self.backend, '_restore_group')
    ...
    def __enter__(self):
        ...
        try:
            ...
            difference = datetime.utcnow() - \
                self.restore_group(self.task_identifier)['date_done']
            if difference < timedelta(seconds=self.timeout):
                self.log.debug('Another instance is running.')
                raise OtherInstanceError(
                    'Failed to acquire lock, {0} already running.'.format(self.task_identifier))
            self.log.debug(
                'Timeout expired, stale lock found, releasing lock.')
    ...
    def is_already_running(self):
        """Returns True if lock exists and has not timed out."""
        date_done = (self.restore_group(self.task_identifier)
                     or dict()).get('date_done')
        if not date_done:
            return False
...
class Celery(CeleryClass):
    ...
    def init_app(self, app):
        ...
        if 'CELERY_RESULT_BACKEND' in app.config:
            self._preconf['CELERY_RESULT_BACKEND'] = app.config[
